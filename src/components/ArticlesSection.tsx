...
const ArticlesSection = () => {
  ...
  return (
    <section id="inspirasjon" style={{ paddingTop: '128px', paddingBottom: '128px', backgroundColor: '#E6EEEB' }}>
          ...
          {latestItems.map((item) => {
            if (item.kind === "media") {
              ...
              return (
                ...
                <h3 className="text-base md:text-lg font-semibold group-hover:text-accent transition-colors line-clamp-2" style={{ lineHeight: 1.3, marginBottom: '12px' }}>
                    {m.title}
                  </h3>
                  <p className="text-sm line-clamp-2 md:line-clamp-3" style={{ color: 'hsl(168, 20%, 28%)', lineHeight: 1.65 }}>
                    {m.excerpt}
                  </p>
                </Link>
              );
